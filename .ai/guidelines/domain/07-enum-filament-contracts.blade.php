...
identity enums (`TeamStatus`, `IdentityType`, `IdentityProvider`, `CredentialsType`,
`IdentityCapability`) all do it. The panel leans on these contracts for badges,
table/infolist columns, `SelectFilter` options, select descriptions and state-gated UI.
...
enum AccessTier: string implements HasColor, HasDescription, HasIcon, HasLabel
{
    ...
    {
        return match ($this) {
            self::None => 'No panel access',
            self::Analyst => 'Day-to-day operator: read and act on records',
            self::Admin => 'Full authority: manage users, roles and settings',
        };
    }
