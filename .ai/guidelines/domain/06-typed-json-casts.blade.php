...
`'collection'`, `AsArrayObject`, `AsCollection`, `AsEnumArrayObject`,
`AsEnumCollection`, `AsEncryptedArrayObject`, and `AsEncryptedCollection`.

The gate is `tests/Arch/NoLooseArrayCastsTest.php` — it reflects over every concrete
...
// BEFORE — blind
'metadata' => 'array',
$identity->metadata['last_projects_sync_at'] ?? null;   // mixed · magic string

// AFTER — typed VO cast (see AsIdentityMetadata → IdentityMetadata in the identity module)
'metadata' => AsIdentityMetadata::class,
$identity->metadata->lastSyncAt(IdentityCapability::Projects);   // ?CarbonImmutable · typed
</code-snippet>

The VO owns shape, validation and accessors (`fromArray()` / `toArray()` / immutable
`withX()` copies). The cast is only the JSON ↔ VO bridge, and its `set()` also accepts a
raw array so legacy assignment stays a reachable, typed branch.

## Rules of thumb

- **Key/format conventions live on ONE owner** (usually an enum accessor — e.g.
  `IdentityCapability::lastSyncedAtKey()`) — never reverse-parse strings to recover a key.
...
- Never introduce `stdClass` in domain code — no domain object produces one.
