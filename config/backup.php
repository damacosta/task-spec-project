...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 *
                 * BACKUP_INCLUDE_PATHS is a comma-separated list of paths relative to storage/ (e.g. "app").
                 * Empty (default) backs up the database only. S3 bucket objects are never captured:
                 * laravel-backup does not read S3 source disks.
                 */
                'include' => array_map(
                    storage_path(...),
                    array_filter(explode(',', (string) env('BACKUP_INCLUDE_PATHS', ''))),
                ),
                ...
                'exclude' => [
                    ...
                    storage_path(sprintf('app/private/%s', $backupName)),
    ...
    'notifications' => [
        /*
         * The backup notification channels are renamed to `spatie.backup.*` by
         * App\Providers\Tools\SpatieBackupServiceProvider to avoid colliding with an
         * app-owned `discord` channel. Point BACKUP_NOTIFIABLE_CHANNEL at the new names,
         * e.g. `spatie.backup.discord`.
         */
        'channels' => $channels = explode(',', (string) env('BACKUP_NOTIFIABLE_CHANNEL', '')),
    ...
    'monitor_backups' => [
        [
            ...
            'health_checks' => [
                ...
                MaximumStorageInMegabytes::class => (int) env('BACKUP_MAX_STORAGE_MB', 50_000),
            ],
        ],
    ],
    ...
    'cleanup' => [
        ...
        'default_strategy' => [
             ...
             */
            'delete_oldest_backups_when_using_more_megabytes_than' => (int) env('BACKUP_MAX_STORAGE_MB', 50_000),
        ],
