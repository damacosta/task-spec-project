...
describe('ExternalIdentity model', static function (): void {
    ...
    test('metadata is always cast to an IdentityMetadata value object', function (): void {
        $identity = ExternalIdentity::factory()->create();

        expect($identity->metadata)->toBeInstanceOf(IdentityMetadata::class)
            ->and($identity->metadata->lastSyncAt(IdentityCapability::Projects))->toBeNull();
    });

    test('metadata round-trips a value object through the database', function (): void {
        $this->travelTo('2026-07-03T12:00:00.000000Z');

        $identity = ExternalIdentity::factory()->create();
        $identity->update([
            'metadata' => $identity->metadata->withSyncedNow(IdentityCapability::Projects),
        ]);

        $fresh = ExternalIdentity::query()->withoutGlobalScopes()->find($identity->id);

        expect($fresh->metadata)->toBeInstanceOf(IdentityMetadata::class)
            ->and($fresh->metadata->lastSyncAt(IdentityCapability::Projects)?->toISOString())
            ->toBe('2026-07-03T12:00:00.000000Z');

        $this->travelTo(date: null);
    });

    test('metadata cast accepts a legacy array assignment', function (): void {
        $identity = ExternalIdentity::factory()->create([
        ...
        expect($fresh->metadata->lastSyncAt(IdentityCapability::Projects)?->toISOString())
