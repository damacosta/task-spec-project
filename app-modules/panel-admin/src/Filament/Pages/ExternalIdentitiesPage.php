...
class ExternalIdentitiesPage extends Page
{
    ...
    /**
     * @return array<int, array{provider: IdentityProvider, identity: ExternalIdentity|null, connected: bool, total_resources: int, mapped_resources: int, unmapped_resources: int, last_synced_at: CarbonImmutable|null}>
     */
        ...
        return collect(IdentityProvider::cases())
            ->map(static function (IdentityProvider $provider) use ($identities): array {
                ...
                return [
                    ...
                    'last_synced_at' => $identity?->metadata->lastSyncAt(IdentityCapability::Projects),
                ];
            })
            ->all();
    }
